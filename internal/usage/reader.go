...
// inputSegmentRows is the minimal row cursor for folding the provider
// prompt-cache split; *sql.Rows satisfies it.
...
// into the summary via addInputSegments.
...
// into the prompt-cache split per period.
...
// CacheOverviewSummary holds exact-cache aggregate statistics over a time period.
...
// CacheOverviewDaily holds exact-cache statistics for a single period.
...
// CacheOverview aggregates exact-cache summary and daily series.
type CacheOverview struct {
	Summary CacheOverviewSummary `json:"summary"`
	Daily   []CacheOverviewDaily `json:"daily"`
}

// UsageReader provides read access to usage data for the control API.
