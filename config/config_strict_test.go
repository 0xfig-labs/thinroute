...
// an empty or comments-only file is an empty overlay, and a correctly indented
// providers: block still loads.
...
`)
		...
		if result.Config.Server.Listen != "127.0.0.1:9999" {
			t.Fatalf("Server.Listen = %q, want the known keys to still apply", result.Config.Server.Listen)
		}
	})
}
