...
func recordRequestRevision(_ *echo.Context, _ auditlog.LoggerInterface, _ string, _ int, _ *ext.Result) {
	// Request revision body capture removed — auditlog is lightweight in-memory only.
}
...
func pinOriginalAuditRequestBody(c *echo.Context, auditLogger auditlog.LoggerInterface) {
	// Body capture removed — auditlog is lightweight in-memory only.
}
