...
func TestResolveProviders_KeepsVertexWithProjectAndLocationUsingDefaultADC(t *testing.T) {
	...
	if _, exists := got["vertex"]; exists {
		t.Fatal("expected vertex not to be resolved without config declaration (env auto-discovery is disabled)")
	}
	if _, exists := filteredRaw["vertex"]; exists {
		t.Fatal("expected raw vertex not to be retained without config declaration (env auto-discovery is disabled)")
	}
}
...
func TestResolveProviders_EndToEnd(t *testing.T) {
	...
	if _, exists := got["anthropic"]; exists {
		t.Error("expected anthropic not to be created from env alone (env auto-discovery is disabled)")
	}
	...
	if _, exists := filteredRaw["anthropic"]; exists {
		t.Error("expected filteredRaw not to contain anthropic from env alone (env auto-discovery is disabled)")
	}
}
...
func TestResolveProviders_EmptyRaw_OnlyEnvVars(t *testing.T) {
	...
	if len(got) != 0 {
		t.Errorf("expected 0 providers with empty raw config, got %d (env auto-discovery is disabled)", len(got))
	}
	if len(filteredRaw) != 0 {
		t.Errorf("expected 0 filtered raw providers with empty raw config, got %d (env auto-discovery is disabled)", len(filteredRaw))
	}
}
