...
// RefreshRuntime handles POST /control/v1/runtime/refresh
...
// ProviderCooldownSnapshot is the API response for the cooldown list endpoint.
type ProviderCooldownSnapshot struct {
...
// ProviderCooldowns handles GET /control/v1/providers/cooldown.
func (h *Handler) ProviderCooldowns(c *echo.Context) error {
	if h.cooldownTracker == nil {
		return c.JSON(http.StatusOK, []ProviderCooldownSnapshot{})
	}

	snapshots := h.cooldownTracker.Snapshot()
	result := make([]ProviderCooldownSnapshot, 0, len(snapshots))
	for _, s := range snapshots {
		result = append(result, ProviderCooldownSnapshot{
