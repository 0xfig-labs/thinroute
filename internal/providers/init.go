...
type InitResult struct {
	...
	// ConfiguredProviders is the effective, control-safe provider inventory keyed
...
//  2. Cache initialization (local, based on config)
...
func Init(ctx context.Context, result *config.LoadResult, factory *ProviderFactory) (*InitResult, error) {
	...
	if count == 0 {
		slog.Warn("no providers configured — add providers via Admin UI")
	}
...
// initCache initializes the local file cache backend.
func initCache(cfg *config.Config) (modelcache.Cache, error) {
	m := cfg.Cache.Model
	cacheDir := ".cache"
	if m.Local != nil && m.Local.CacheDir != "" {
