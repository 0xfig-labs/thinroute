...
type Target struct {
	...
	// FallbackOnly marks this target as usable only when no non-fallback target
	// is available. Primary targets are tried first; if all are unavailable,
	// fallback targets are used.
	FallbackOnly bool `json:"fallback_only,omitempty" bson:"fallback_only,omitempty"`
}
...
type VirtualModel struct {
	...
	// of the same Source and are read-only to the control API.
...
const (
	...
	StrategyCost = "cost"
	// StrategyWeighted selects targets proportionally to their configured Weight.
	// Higher weight = more traffic.
	StrategyWeighted = "weighted"
	// StrategyRandom picks a target uniformly at random.
	StrategyRandom = "random"
	// StrategyLeastUsed selects the target with the fewest in-flight requests,
...
// Role kinds for the control view.
const (
	KindRedirect = "redirect"
	KindPolicy   = "policy"
)

// View is the control-facing representation of one virtual model.
type View struct {
