...
func TestExpandString(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "${THINROUTE_MASTER_KEY:-}",
			envVars:  map[string]string{"THINROUTE_MASTER_KEY": "secret-key"},
			expected: "secret-key",
		},
		{
...
func TestApplyEnvOverrides(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "bool overrides",
			envVars: map[string]string{"METRICS_ENABLED": "true", "LOGGING_ENABLED": "1"},
			check: func(t *testing.T, cfg *Config) {
		...
		},
		{
			...
			check: func(t *testing.T, cfg *Config) {
				if cfg.Server.Listen != "127.0.0.1:52180" {
					t.Errorf("Server.Listen = %q, want %q", cfg.Server.Listen, "127.0.0.1:52180")
				}
