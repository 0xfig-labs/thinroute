...
func TestRunHealthAndReadyDispatch(t *testing.T) {
	// Write a temporary config.yaml to set the listen address — config.Load()
	// reads this from the working directory, and ServerConfig.Listen has no
	// env tag (the old PORT env binding was removed with MasterKey/auth).
	tmpDir := t.TempDir()
	t.Chdir(tmpDir)
	...
	if err := os.WriteFile(filepath.Join(tmpDir, "config.yaml"), []byte(cfgContent), 0644); err != nil {
