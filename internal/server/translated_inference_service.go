...
type translatedInferenceService struct {
	...
	disableReasoningModels   map[string]struct{}
...
func (s *translatedInferenceService) newInferenceOrchestrator() *gateway.InferenceOrchestrator {
	...
	// rateLimiter removed; RouteGate stays nil
...
func (s *translatedInferenceService) dispatchChatCompletion(c *echo.Context, req *core.ChatRequest, workflow *core.Workflow) error {
	...
	if s.budgetChecker != nil {
		if err := enforceBudgetForContext(c.Request().Context(), s.budgetChecker); err != nil {
...
func prepareChatCompletionRequest(
	...
	ctx, preparedReq, workflow, err := unpackPrepared(ctx, prepared, err, chatPreparedFields)
	if err == nil && s.shouldDisableReasoning(workflow) {
		disableChatReasoning(preparedReq)
	}
...
func prepareResponsesRequest(
	...
	if s.shouldDisableReasoning(workflow) {
		disableResponsesReasoning(preparedReq)
	}
...
var reasoningExtraFields = []string{
	"reasoning_effort",
	"thinking",
	"enable_thinking",
	"thinking_budget",
...
func normalizeReasoningDisabledModels(models []string) map[string]struct{} {
	if len(models) == 0 {
		return nil
	}
...
func (s *translatedInferenceService) shouldDisableReasoning(workflow *core.Workflow) bool {
	if len(s.disableReasoningModels) == 0 || workflow == nil {
		return false
	}
	for _, model := range []string{workflow.RequestedQualifiedModel(), workflow.ResolvedQualifiedModel()} {
		if _, disabled := s.disableReasoningModels[strings.ToLower(strings.TrimSpace(model))]; disabled {
			return true
		}
	}
	return false
...
func disableChatReasoning(req *core.ChatRequest) {
	if req == nil {
		return
	}
	req.Reasoning = nil
	req.ExtraFields = core.RemoveUnknownJSONFields(req.ExtraFields, reasoningExtraFields...)
}

func disableResponsesReasoning(req *core.ResponsesRequest) {
	if req == nil {
		return
	}
	req.Reasoning = nil
...
func (s *translatedInferenceService) handleStreamingReadCloser(
	...
	if err := flushStream(c.Response(), wrappedStream); err != nil {
		recordStreamingError(model, provider, c.Request().URL.Path, requestID, c.Request().Context(), err)
	}
	return nil
}
...
func recordStreamingError(model, provider, path, requestID string, ctx context.Context, err error) {
