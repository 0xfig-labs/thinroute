...
// CooldownTracker returns the router's cooldown tracker for control visibility.
func (r *Router) CooldownTracker() *CooldownTracker {
	return r.cooldown
}
...
func routeResolvedModelCall[Req any, Resp any](
	...
	// Check provider-level cooldown before making the call.
	providerName := selector.Provider
	if r.cooldown != nil {
		status := r.cooldown.Check(CooldownKey{
			ProviderName: providerName,
			Scope:        "provider",
		})
		if status.Active {
			var zero Resp
			return zero, "", core.NewRateLimitError(providerName, fmt.Sprintf("provider %q is cooling down until %s", providerName, status.CoolingUntil.Format(time.RFC3339)))
		}
		// Also check model-level lockout.
		if selector.Model != "" {
			modelStatus := r.cooldown.Check(CooldownKey{
				ProviderName: providerName,
			...
			if modelStatus.Active {
				var zero Resp
				return zero, "", core.NewRateLimitError(providerName, fmt.Sprintf("model %q on provider %q is locked out until %s", selector.Model, providerName, modelStatus.CoolingUntil.Format(time.RFC3339)))
			}
		}
	}
	...
	// Record cooldown for provider-level errors (429, 401, 403).
	if err != nil && r.cooldown != nil {
		var gwErr *core.GatewayError
		if errors.As(err, &gwErr) {
			errMsg := ""
			if gwErr.Err != nil {
				errMsg = gwErr.Err.Error()
			}
			RecordFailureFromStatusCode(r.cooldown, providerName, 0, selector.Model, gwErr.StatusCode, 0, errMsg)
		}
	}
