...
type Service struct {
	...
	// the same source, and are read-only to the control API.
	...
	// leastUsed tracks in-flight request counts per target for the
	// least_used load-balancing strategy.
	leastUsed LeastUsedBalancer

	// healthSnapshots returns runtime health diagnostics for configured
	// providers, consulted by the health_aware strategy. Defaults to returning
	// an empty slice when not set.
	healthSnapshots func() []providers.ProviderRuntimeSnapshot
}

// SetHealthSnapshots installs the provider health snapshot source consulted by
// the health_aware load-balancing strategy. Must be called before the service
// starts resolving requests.
func (s *Service) SetHealthSnapshots(snapshots func() []providers.ProviderRuntimeSnapshot) {
	if s == nil {
		return
	}
	s.healthSnapshots = snapshots
}
...
func NewService(store Store, catalog Catalog, defaultEnabled bool) (*Service, error) {
	...
	service := &Service{
		...
		healthSnapshots: func() []providers.ProviderRuntimeSnapshot { return nil },
	}
...
// ListViews returns all virtual models (redirects and policies) for the control clients.
...
// redirectViewResolution summarizes a redirect for the control view: a
...
// check. The control API runs against a warm catalog, so a target it cannot serve
...
// managedSourceError is returned when the control API tries to write a virtual
...
func managedSourceError(source string) error {
	return newValidationError(fmt.Sprintf(
		"virtual model %q is managed by config.yaml or VIRTUAL_MODELS and cannot be changed from the control API; edit your configuration instead",
		source), nil)
}
