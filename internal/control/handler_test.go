...
func TestUsageSummary_NilReader(t *testing.T) {
	...
	c, rec := newHandlerContext("/control/v1/usage/summary")
...
func TestUsageSummary_Success(t *testing.T) {
	...
	c, rec := newHandlerContext("/control/v1/usage/summary?days=30")
...
func TestDailyUsage_NilReader(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/usage/daily")
...
func TestDailyUsage_Success(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/daily?days=7")
...
func TestUsageByModel_NilReader(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/usage/models")
...
func TestUsageByModel_Success(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/models?days=30")
...
func TestUsageByUserPath_NilReader(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/usage/user-paths")
...
func TestUsageByUserPath_Success(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/user-paths?days=30")
...
func TestUsageByLabel_NilReader(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/usage/labels")
...
func TestUsageByLabel_Success(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/labels?days=30")
...
func TestUsageLog_NilReader(t *testing.T) {
	...
	// limit=0 (which 400s).
	c, rec := newHandlerContext("/control/v1/usage/log")
...
func TestUsageLog_Success(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/log?days=30&limit=50&offset=0")
...
func TestUsageLog_PreservesProviderName(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/log?days=30")
...
func TestUsageLog_WithFilters(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/log?model=gpt-4&provider=openai&user_path=/team&label=team-alpha&search=test&limit=10&offset=5")
...
func TestListModels_NilRegistry(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/models")
...
func TestListModels_WithCategoryFilter(t *testing.T) {
	...
	t.Run("FilterTextGeneration", func(t *testing.T) {
		c, rec := newHandlerContext("/control/v1/models?category=text_generation")
		if err := h.ListModels(c); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if rec.Code != http.StatusOK {
	...
	t.Run("FilterAll", func(t *testing.T) {
		c, rec := newHandlerContext("/control/v1/models?category=all")
		if err := h.ListModels(c); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		var models []providers.ModelWithProvider
...
func TestListModels_InvalidCategory(t *testing.T) {
	...
	h := NewHandler(nil, registry)
	c, rec := newHandlerContext("/control/v1/models?category=bogus_category")
...
func TestListCategories_NilRegistry(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/models/categories")
...
func TestProviderStatus_DistinguishesProvidersWithSameTypeByName(t *testing.T) {
	...
	c, rec := newHandlerContext("/control/v1/providers/status")
...
func TestRuntimeConfig_ReturnsAllowlistedRuntimeFlags(t *testing.T) {
	h := NewHandler(nil, nil, WithRuntimeConfig(RuntimeConfigResponse{
		LoggingEnabled: "on",
		UsageEnabled:   "off",
		CacheEnabled:   "on",
	}))
	c, rec := newHandlerContext("/control/v1/runtime/config")

	if err := h.RuntimeConfig(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rec.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", rec.Code)
...
func TestRefreshRuntime_ReturnsReport(t *testing.T) {
	...
	c, rec := newHandlerContext("/control/v1/runtime/refresh")
...
func TestCacheOverview_FeatureUnavailableWhenCacheDisabled(t *testing.T) {
	h := NewHandler(&mockUsageReader{}, nil, WithRuntimeConfig(RuntimeConfigResponse{
		CacheEnabled: "off",
	}))
	c, rec := newHandlerContext("/control/v1/cache/overview")
...
func TestCacheOverview_ReturnsPayloadWhenEnabled(t *testing.T) {
	reader := &mockUsageReader{
		cacheOverview: &usage.CacheOverview{
			...
			Daily: []usage.CacheOverviewDaily{{Date: "2026-03-31", Hits: 4, ExactHits: 3, InputTokens: 120, OutputTokens: 60, TotalTokens: 180}},
		},
	}
	h := NewHandler(reader, nil, WithRuntimeConfig(RuntimeConfigResponse{
		CacheEnabled: "on",
	}))
	c, rec := newHandlerContext("/control/v1/cache/overview?days=30&user_path=/team")
...
func TestCacheOverview_ReturnsErrorWhenReaderFails(t *testing.T) {
	...
	c, rec := newHandlerContext("/control/v1/cache/overview?days=30")
...
func TestHandleError_DoesNotLogCanceledRequestsAtDefaultLevel(t *testing.T) {
	...
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/control/v1/cache/overview", nil)
...
func TestHandleError_LogsClientErrorsAtWarnLevel(t *testing.T) {
	...
	e := echo.New()
	req := httptest.NewRequest(http.MethodPost, "/control/v1/workflows", nil)
	...
	if !strings.Contains(logOutput, `"msg":"control request failed"`) {
		t.Fatalf("expected control request failed log, got %q", logOutput)
	}
	if !strings.Contains(logOutput, `"path":"/control/v1/workflows"`) {
		t.Fatalf("expected         control path in log, got %q", logOutput)
	}
...
func TestHandleError_LogsServerErrorsAtErrorLevel(t *testing.T) {
	...
	e := echo.New()
	req := httptest.NewRequest(http.MethodPut, "/control/v1/guardrails", nil)
...
func TestParseUsageParams_UsesTimezoneHeaderForDefaultRange(t *testing.T) {
	...
	c := newContext("")
	c.Request().Header.Set(usageTimeZoneHeader, "Europe/Warsaw")
...
func TestTokenThroughput_InvalidGranularity(t *testing.T) {
	...
	c, rec := newHandlerContext("/control/v1/usage/throughput?granularity=weekly")
...
func TestTokenThroughput_MissingGranularity(t *testing.T) {
	h := NewHandler(&mockUsageReader{}, nil)
	c, rec := newHandlerContext("/control/v1/usage/throughput")
	if err := h.TokenThroughput(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rec.Code != http.StatusBadRequest {
...
func TestTokenThroughput_NilReaderReturnsEmptyWindow(t *testing.T) {
	h := NewHandler(nil, nil)
	c, rec := newHandlerContext("/control/v1/usage/throughput?granularity=minute")
	if err := h.TokenThroughput(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rec.Code != http.StatusOK {
...
func TestTokenThroughput_ForwardsTimezoneOffset(t *testing.T) {
	...
	e := echo.New()
	req := httptest.NewRequest(http.MethodGet, "/control/v1/usage/throughput?granularity=day", nil)
...
func TestTokenThroughput_GatewayError(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/throughput?granularity=hour")
	if err := h.TokenThroughput(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rec.Code != http.StatusBadGateway {
...
func TestTokenThroughput_GenericError(t *testing.T) {
	...
	h := NewHandler(reader, nil)
	c, rec := newHandlerContext("/control/v1/usage/throughput?granularity=day")
	if err := h.TokenThroughput(c); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rec.Code != http.StatusInternalServerError {
