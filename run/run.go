...
func Run(ctx context.Context, opts Options) error {
	...
	result, err := config.Load(cliOpts.Config)
	if err != nil {
	...
	addr := result.Config.Server.Listen

	// Warn when listening on a non-loopback address without authentication
	if host, _, err := net.SplitHostPort(addr); err == nil {
		...
		if !isLoopback {
			slog.Warn("listening on non-loopback address without authentication", "address", addr)
		}
	}
