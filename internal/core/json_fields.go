...
// RemoveUnknownJSONFields returns fields without the named top-level members.
func RemoveUnknownJSONFields(fields UnknownJSONFields, names ...string) UnknownJSONFields {
	if fields.IsEmpty() || len(names) == 0 {
		return fields
	}
	...
	buf.WriteByte('{')
	wrote := false
	if err := appendUnknownJSONMembers(buf, fields.raw, excluded, &wrote); err != nil {
		return fields
	}
	buf.WriteByte('}')
