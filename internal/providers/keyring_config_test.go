...
func TestResolveProviders_NumberedAPIKeyEnvVars(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, _ := resolveProviders(map[string]config.RawProviderConfig{}, globalResilience, testDiscoveryConfigs)
			if _, exists := got["openai"]; exists {
				t.Error("expected no provider from env vars alone (env auto-discovery is disabled)")
			}
		})
	}
}
...
func TestResolveProviders_NumberedKeysOnSuffixedProviders(t *testing.T) {
	t.Run("numbered key on a suffixed provider", func(t *testing.T) {
		...
		got, _ := resolveProviders(map[string]config.RawProviderConfig{}, globalResilience, testDiscoveryConfigs)
		if _, exists := got["openai-eu"]; exists {
			t.Error("expected no provider from env vars alone (env auto-discovery is disabled)")
		}
	})
	...
	t.Run("suffix ending in a digit is not a rotation slot", func(t *testing.T) {
		...
		got, _ := resolveProviders(map[string]config.RawProviderConfig{}, globalResilience, testDiscoveryConfigs)
		if _, exists := got["openai-region-2"]; exists {
			t.Error("expected no provider from env vars alone (env auto-discovery is disabled)")
		}
	})
}
