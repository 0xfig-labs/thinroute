...
// implementation may carry its own state (in-flight counters, health snapshots)
// or be stateless.
type Balancer interface {
	NextTarget(targets []resolvedTarget) int
}
...
//
// Targets are split into two pools: primary (FallbackOnly=false) and fallback
// (FallbackOnly=true). Primary targets are preferred; if none are available,
// fallback targets are used.
func (s *Service) balancedResolution(entry redirectEntry) (core.ModelSelector, bool) {
	...
	// Split into primary and fallback pools
	primary := supported[:0]
	var fallback []resolvedTarget
	...
	// Prefer targets with live rate-limit capacity.
	viable := s.targetsWithCapacity(pool)
	if len(viable) == 0 {
		viable = pool[:1]
	}
	if len(viable) == 1 {
	...
	case StrategyWeighted:
		index := (&WeightedBalancer{}).NextTarget(viable)
		return viable[index].selector, true
	case StrategyRandom:
		index := (&RandomBalancer{}).NextTarget(viable)
		return viable[index].selector, true
	case StrategyLeastUsed:
		index := s.leastUsed.NextTarget(viable)
		return viable[index].selector, true
	case StrategyHealthAware:
		snapshots := s.healthSnapshots()
		index := (&HealthAwareBalancer{Snapshots: snapshots}).NextTarget(viable)
		return viable[index].selector, true
	default: // StrategyRoundRobin
		index := weightedIndex(viable, s.balancer.next(entry.vm.Source))
		return viable[index].selector, true
	}
}
