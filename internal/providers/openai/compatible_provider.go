...
type CompatibleProviderConfig struct {
	...
	// ChatRequestBody overrides the default request body serialization for
	// ChatCompletion and StreamChatCompletion. When nil, the default
	// package-level chatRequestBody is used (which applies OpenAI reasoning-
	// parameter adaptation for o-series and gpt-5 models). Set this to a
	// pass-through function for generic compatible endpoints.
	ChatRequestBody func(*core.ChatRequest) (any, error)
}
...
func NewCompatibleProvider(apiKey string, opts providers.ProviderOptions, cfg CompatibleProviderConfig) *CompatibleProvider {
	...
	if cfg.ChatRequestBody != nil {
		p.chatRequestBody = cfg.ChatRequestBody
	} else {
		p.chatRequestBody = chatRequestBody
	}
...
func (p *CompatibleProvider) ChatCompletion(ctx context.Context, req *core.ChatRequest) (*core.ChatResponse, error) {
	...
	body, err := p.chatRequestBody(adapted)
	if err != nil {
		return nil, err
	}
...
func (p *CompatibleProvider) StreamChatCompletion(ctx context.Context, req *core.ChatRequest) (io.ReadCloser, error) {
	...
	body, err := p.chatRequestBody(streamReq)
	if err != nil {
		return nil, err
	}
