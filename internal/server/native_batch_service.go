...
func (s *nativeBatchService) batch() *gateway.BatchOrchestrator {
	...
	s.orchestrator = gateway.NewBatchOrchestrator(gateway.BatchConfig{
		...
		BudgetEnforcer:                       batchAdmissionEnforcer(s.budgetChecker),
	})
	return s.orchestrator
}
...
// batchAdmissionEnforcer gates batch submission on budgets.
func batchAdmissionEnforcer(checker BudgetChecker) func(context.Context) error {
	if checker == nil {
		return nil
	}
