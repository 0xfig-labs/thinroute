...
func TestRunHealthProbe_UsesConfiguredPortAndBasePath(t *testing.T) {
	...
	endpoint := healthProbeURL(srvCfg)
	if err := checkHealthEndpoint(context.Background(), &http.Client{Timeout: time.Second}, endpoint); err != nil {
		t.Fatalf("health check error = %v, want nil", err)
	}
}
