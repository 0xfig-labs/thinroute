...
func TestBatchBudgetEnforcerUsesResolvedWorkflow(t *testing.T) {
	checker := &countingBudgetChecker{}
	enforcer := batchAdmissionEnforcer(checker)
	if enforcer == nil {
...
type errorBudgetChecker struct {
	BudgetChecker
}

func (c *errorBudgetChecker) Check(_ context.Context, _ string, _ time.Time) error {
	return errors.New("budget exceeded")
}

func TestBatchAdmissionEnforcerAppliesBudgetCheck(t *testing.T) {
	checker := &countingBudgetChecker{}
	enforcer := batchAdmissionEnforcer(checker)
	if enforcer == nil {
		t.Fatal("batchAdmissionEnforcer() = nil, want function")
	...
	if err := enforcer(ctx); err != nil {
		t.Fatalf("batch submission rejected: %v", err)
	}
	if checker.calls != 1 {
		t.Fatalf("budget checker was called %d times, want 1", checker.calls)
	}

	// Verify budget errors are wrapped as GatewayErrors
	errorEnforcer := batchAdmissionEnforcer(&errorBudgetChecker{})
	var gatewayErr *core.GatewayError
	if !errors.As(errorEnforcer(ctx), &gatewayErr) {
		t.Fatal("expected a GatewayError from the budget check")
	}
}
